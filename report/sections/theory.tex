...
$K_v=\SI{1.05}{\micro s/(px/s)}$，第三题自动序列运行期间切换到
...
\begin{itemize}
  \item 加速度前馈：根据车辆纵向加速度直接补偿起步和制动扰动，
        ...
        $K_{\mathrm{AF}}=\SI{2.4}{\micro s/mg}$，并经\SI{30}{\%}权重
  ...
  \item 偏航角速度前馈：右转时车辆偏航给球引入离心惯性，按偏航角速度
        ...
        $K_{\mathrm{Y}}=\SI{-0.45}{\micro s/(\deg/s)}$，
...
\proposed{上述前馈增益目前按单圈实测粗调，尚未做系统性的参数整定；
最终增益应在正式赛道按R4至R6判定重新标定。}
